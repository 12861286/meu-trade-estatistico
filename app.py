...
# --- FUNÇÃO PARA LIMPAR COLUNAS YFINANCE ---
...
with col_cfg2:
    gap_digitado = st.number_input("GAP desejado (%):", value=float(gap_atual), step=0.1)
with col_cfg3:
    ...
    with st.spinner(f'Processando {ativo}...'):
        ...
        if not df.empty and len(df) > 10:
            ...
            if len(eventos_digitados) >= 3:
                ...
                col_m1.metric("Probabilidade", f"{x_dig}%")
                col_m2.metric("Alvo Sugerido", f"{y_dig}%")
                col_m3.metric("Ocorrências", len(eventos_digitados))
                
                qtd_pos = len(eventos_digitados[eventos_digitados['Resultado_Fechamento'] > 0])
                perc_pos = round((qtd_pos / len(eventos_digitados)) * 100, 1)
                st.write(f"**Tendência de Fechamento:** Positivo {perc_pos}% | Negativo {round(100 - perc_pos, 1)}%")
            else:
                st.warning("Poucas ocorrências para este GAP específico.")

            # 📋 Mapa de GAPs
            st.markdown("---")
            st.subheader("📋 Mapa de GAPs (Frequência)")
            ranking = []
            for val in [x * 0.5 for x in range(-10, 11)]:
                ...
                if len(ev_r) >= 4:
                    ...
                    ranking.append({
                        ...
                        "Alvo": f"{y_r}%", 
                        "Acerto": f"{x_r}%", 
                        "Máx Média": f"{ev_r['Max_Apos_Abertura'].mean():.2f}%"
                    })
            if ranking:
                st.table(pd.DataFrame(ranking).sort_values(by="GAP", ascending=False))

            # 🚀 Radar simplificado (Evita bloqueio do Yahoo)
            st.markdown("---")
            st.subheader(f"🚀 Radar de Elite (> {filtro_radar}% Acerto)")
            st.info("O radar analisa os ativos da lista que abriram com GAPs estatisticamente fortes HOJE.")
            # Aqui você pode adicionar a lógica de radar otimizada se desejar
            
            # 📊 Gráfico
            st.subheader(f"📊 Volatilidade Pós-Abertura - {ativo}")
            st.plotly_chart(px.bar(df.tail(50), y=['Max_Apos_Abertura', 'Queda_Apos_Abertura'], barmode='group'), use_container_width=True)
        else:
