...
# --- FUNÇÃO DE DOWNLOAD REFORÇADA (CORRIGIDA) ---
def download_seguro(ticker, start=None, period=None):
    for tentativa in range(2):
        try:
            ...
            if not df.empty:
                # Limpa colunas Multi-Index (Proteção contra erro de coluna)
                df.columns = [c[0] if isinstance(c, tuple) else c for c in df.columns]
                # GARANTE QUE OS DADOS SÃO NÚMEROS (Proteção contra TypeError)
...
# --- CONTROLES PRINCIPAIS (VISUAL ORIGINAL) ---
    ...
    with st.spinner(f'Processando {ativo}...'):
        ...
        if not df.empty and len(df) > 10:
            ...
            if rodar:
                ...
                # --- RADAR ORIGINAL COM PAUSA ANTI-BLOQUEIO ---
                st.markdown("---")
                st.subheader(f"🚀 Radar de Elite (> {filtro_radar}% Acerto)")
                radar_hoje = []
                progresso_radar = st.progress(0)
                
                for i, ticker in enumerate(lista_sugerida[:35]): # Limite de 35 para segurança
                    progresso_radar.progress((i + 1) / 35)
                    try:
                        ...
                        time.sleep(1.2) # Pausa estratégica para evitar "Bloqueado"
                    except:
                        continue
